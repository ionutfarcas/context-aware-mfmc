...
if __name__ == '__main__':
	...
	obj_ML 		= lambda k_a_1, k_c_1, n, p: ( k_a_1 + k_c_1*acc_rate_ML(n) + cost_rate_ML(n) )/ (p - n)
	obj_ML_grad = lambda k_a_1, k_c_1, n, p: (k_c_1*d_acc_rate_ML(n) + d_cost_rate_ML(n) ) / (p - n) + \
								( k_a_1 + k_c_1*acc_rate_ML(n) + cost_rate_ML(n)  ) / (p - n)**2
	...
	for i, p in enumerate(budget):
		...
		k_a_1 = acc_rate_SG(n_star_SG[i])
		k_c_1 = cost_rate_SG(n_star_SG[i])

		obj_ML_f 		= lambda n: 1e2*new_p*obj_ML(k_a_1, k_c_1, n, new_p)
		obj_ML_f_grad 	= lambda n: 1e2*new_p*obj_ML_grad(k_a_1, k_c_1, n, new_p)
	...
	np.save('results/ML_after_SG_n_star.npy', n_star_ML)
